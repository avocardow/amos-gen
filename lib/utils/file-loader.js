...
async function loadTemplateFiles(options = {}) {
  ...
  if (!options.simple) {
    ...
    templates['.cursor/rules/amos/communication/agent_coordination.mdc'] = await fs.readFile(path.join(communicationDir, 'agent_coordination.mdc'), 'utf8');
  } else {
  ...
  templates['scripts/workspace_isolation.sh'] = await fs.readFile(path.join(scriptsDir, 'workspace_isolation.sh'), 'utf8');
