...
# Start agents with bootstrap awareness
tmux send-keys -t "$SESSION_NAME:MANAGER" "echo '🚀 MANAGER: First read BOOTSTRAP.mdc' && ./scripts/agent_sanity_check.sh" C-m
tmux send-keys -t "$SESSION_NAME:PLANNER" "echo '🚀 PLANNER: First read BOOTSTRAP.mdc' && ./scripts/agent_sanity_check.sh" C-m
tmux send-keys -t "$SESSION_NAME:WORKER" "echo '🚀 WORKER: First read BOOTSTRAP.mdc' && ./scripts/agent_sanity_check.sh" C-m
...
echo "   • State: .cursor/rules/amos/project-data/system_state.mdc"
echo ""
