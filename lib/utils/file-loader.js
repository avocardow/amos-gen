...
async function loadTemplateFiles(options = {}) {
  ...
  // Technology-specific best practices with Context7 MCP integration
  templates['.cursor/rules/amos/project-data/tech_stack_practices.mdc'] = await fs.readFile(path.join(projectDataDir, 'tech_stack_practices.mdc'), 'utf8');
  ...
  templates['scripts/detect_tech_stack.sh'] = await fs.readFile(path.join(scriptsDir, 'detect_tech_stack.sh'), 'utf8');
