...
async function generateTemplateFiles(projectPath, options = {}) {
  try {
    const { loadTemplateFiles } = require('../utils/file-loader');
    const templates = await loadTemplateFiles(options);
    ...
    // Add QUICKSTART.md for simple mode
    if (options.simple) {
      const quickstartPath = path.join(projectPath, 'QUICKSTART.md');
      const quickstartContent = await fs.readFile(path.join(__dirname, '..', '..', 'QUICKSTART.md'), 'utf8');
